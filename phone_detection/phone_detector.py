...
import sys
import os
curr_dir = "/home/user/phone_detection_ros2/src/phone_detection/phone_detection"
...
class PhoneDetector(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(String, 'mobile_detection_topic', 10)
        self.subscriber_ = self.create_subscription(Image, '/camera/image', self.image_callback, 10)
        self.timer = self.create_timer(0, self.detect_phone)
        # Initialize YOLO model
        self.yolo = YoloV3()
        load_darknet_weights(self.yolo, 'models/yolov3.weights') 
        self.frame = None
            ...
            if (self.succeed_call):
                ...
                for i in range(nums[0]):
                    if int(classes[0][i] == 67):
                        ...
                        self.get_logger().info('Mobile Phone detected')
                        break
...
#     phone_detect_pub = phoneDetector()
